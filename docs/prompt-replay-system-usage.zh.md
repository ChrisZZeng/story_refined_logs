...
今天 Workbench 增加了几项面向日常调 prompt 的优化：

- Configure 页新增并发设置：`Concurrent Replay Attempts` 默认 `20`，`Concurrent Judge Requests` 默认 `50`。
- 点击 `Run` 后会创建后台 replay job；刷新页面后，只要 Workbench server 进程还在，页面会自动恢复运行中的 job 并继续轮询结果。
- 左侧新增 `Replay History`，会扫描当前 `Log Group Dir` 下的历史 replay 产物，点击后可以回看对应的 Replay / Judge 结果。
- 多次点击 `Run` 时，Workbench 任务层仍然按队列串行执行；单个 job 内部的 replay / judge 并发由上面的两个并发参数控制。
...
Setup 中的 `Follow Badcase Commit` 默认开启，表示 replay 代码版本跟随原始 badcase 日志的 commit。关闭后不创建/使用该 commit 的 managed worktree，而是直接使用 `Oreturn Repo` 当前 `HEAD` 运行；这种模式适合临时验证当前分支代码，但结论不再严格代表原始 badcase 代码版本。

## 推荐工作流

1. 从 consistency review 里选出要验证的 badcase turns。
2. 启动 Workbench。
3. 在 Configure 页填写日志、turns、repeats、并发、模型和 `oreturn` 路径。
4. 在上方查看每个 badcase 的 issue、原始输出、replay 输出和 judge。
5. 在下方选择 prompt source，修改 Draft。
6. 点击 Run，等待 replay 和 judge 完成。
7. 用左侧进度面板检查每个 turn / run 的状态，也可以从 Replay History 回看历史任务。
...
| `Follow Badcase Commit` | 默认开启；关闭后 replay 使用 `Oreturn Repo` 当前 `HEAD`。 |
| `Concurrent Replay Attempts` | replay attempt 的全局并发上限。Workbench 默认 `20`，所有 turns 和 repeats 共用这个上限。 |
| `Concurrent Judge Requests` | judge 请求的全局并发上限。Workbench 默认 `50`，Regression Consistency Judge 和每个 issue judge 共用这个上限。 |
| `Replay Model` | 重放小说系统时使用的模型。 |
| `Judge Model` | 判断 issue 是否修复时使用的模型。 |

并发字段会写入临时 task snapshot：
...
`replayAttempts` 控制所有 turn / repeat 加起来同时进入 `runOreturnReplay` 的数量；`judgeRequests` 控制所有 regression consistency judge 和 issue judge 加起来同时请求模型的数量。Workbench 页面上的多个 `Run` job 仍然串行排队，避免多个完整 replay job 同时抢占同一组日志和模型资源。
...
- 左侧 `CASE CONTEXT`：turn 切换、replay 进度和历史任务回看。
...
左侧 `Replay History` 会扫描当前 `Log Group Dir` 下的历史结果目录：

```text
logs/<branch-version>/prompt-patch-replay/<replay-id>/
```

每个历史项会显示 replay id、通过 run 数、整体通过率、turn 数和更新时间。点击一个已完成的历史项后，Workbench 会读取该结果目录中的 `summary.json`，并恢复上方 `Replay` / `Judge` tabs 的可查看状态。

注意：

- History 只依赖已经落盘的 replay 产物，所以关闭浏览器后再打开也可以回看完成过的任务。
- 如果某个目录还没有 `summary.json`，会显示为 incomplete，通常表示任务中断或仍未完成。
- 刷新页面时，如果 Workbench server 进程内还有 queued / running job，页面会优先恢复该运行中任务；否则会默认加载最近一个 completed 历史结果。
- 如果 Workbench server 进程已经退出，运行中的 job 不会继续执行；只能回看已经写出到结果目录的 completed / incomplete 产物。
...
Run 会先创建一个 Workbench replay job，接口会返回类似 `job-000001` 的 job id。浏览器页面会轮询：

```text
GET /api/replay/jobs/<job-id>
```

刷新页面后，Workbench 会先调用：
