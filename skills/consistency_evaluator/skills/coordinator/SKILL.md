...
一次评测目录约定为 `logs/<branch+version>/`：文件化运行日志放在 `run_logs/<run-id>/`，一致性评测结果放在 `consistency-review/<run-id>/`。Coordinator 不直接调用 `oreturn`；如果输入来自 `oreturn`，先用单独导出步骤生成 `logs/<branch+version>/run_logs/<run-id>`。

1. 从运行目录生成玩家可见时间线：

   ```bash
   node <coordinator-skill-dir>/scripts/build-visible-timeline.mjs logs/<branch+version>/run_logs/<run-id> --out logs/<branch+version>/consistency-review/<run-id>/visible-timeline.jsonl
   ```

2. 生成批次计划和每个批次的任务文件：

   ```bash
   node <coordinator-skill-dir>/scripts/plan-batches.mjs logs/<branch+version>/consistency-review/<run-id>/visible-timeline.jsonl --out-dir logs/<branch+version>/consistency-review/<run-id> --run-dir logs/<branch+version>/run_logs/<run-id>
   ```
