  ...
  return async function handleWorkbenchApi(request) {
    try {
      ...
      return jsonResponse(
        ...
        { status: ['WORKBENCH_TASK_NOT_CONFIGURED', 'WORKBENCH_CONFIG_INVALID'].includes(error?.code) ? 400 : 500 },
      );
    }
  };
}
...
function promptSourcesForCall({ turn, call, callIndex, stage }) {
...
function observedPromptSource({ turn, callIndex, stage, callKind, fieldPath, text, label }) {
